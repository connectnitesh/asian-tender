...
const Addtender = () => {
  ...
  useEffect(() => {
    setLoad(true);
    const updatetID = async () => {
      const getTender = await axios.get("http://localhost:4000/api");
      const len = getTender.data.length;
      console.log(getTender.data);
      const lastID = getTender.data[len - 1].tID;
      setTender({ ...tender, tID: lastID + 1 });
    };
    updatetID();
  }, [tender]);
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await axios.post("http://localhost:4000/api", tender);

    if (res.data.success === false) {
      alert(res.data.message);
    } else {
      alert("Tender added successfully!");
      setTender({ tID: tender.tID + 1 });
    }
    handleReset();
  };
